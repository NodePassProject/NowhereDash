...
export default function EndpointsPage() {
  ...
  const {
    isOpen: isGuidedAddOpen,
    onOpen: onGuidedAddOpen,
    onOpenChange: onGuidedAddOpenChange,
  } = useDisclosure();
  const openBlankAddModal = onAddOpen;
  const {
    isOpen: isDeleteOpen,
    onOpen: onDeleteOpen,
  ...
  const handleReconnect = async (endpointId: number) => {
    try {
      ...
      addToast({
        ...
        description: result.message || t("toast.reconnectSuccessDesc"),
        color: "success",
      });
  ...
  const handleConnect = async (endpointId: number) => {
    try {
      ...
      addToast({
        ...
        description: result.message || t("toast.connectSuccessDesc"),
        color: "success",
      });
  ...
  const handleExportData = async () => {
    try {
      ...
      a.href = url;
      a.download = `nowheredash-${new Date().toISOString().split("T")[0]}.json`;
    ...
    if (over && active.id !== over.id) {
      setSortedValidateResults((items) => {
        const oldIndex = items.findIndex(
          (item, idx) => `item-${idx}` === active.id,
        );
        const newIndex = items.findIndex(
          (item, idx) => `item-${idx}` === over.id,
        );
  ...
  const handleConfirmImport = async () => {
    ...
    const importableEndpoints = sortedValidateResults
      ...
      .map((result) => ({
        ...
        apiKey:
          importFileData?.data?.endpoints?.find(
        ...
        <div className="h-full w-full items-start justify-center overflow-scroll px-4 pb-24 pt-8">
          <div className="space-y-4">
            ...
            <div className="space-y-3">
              ...
              <div className="flex items-center gap-3">
                <span className="text-small text-default-500">
                  {t("status.instanceCount")}:
                </span>
                <Chip color="primary" size="sm" variant="flat">
                  {t("table.instancesCount", {
                    count: realTimeData.tunnelCount,
                  })}
                </Chip>
              </div>
  ...
  const handleRename = async (newName: string) => {
    ...
    try {
      ...
      addToast({
        ...
        description:
          error instanceof Error ? error.message : t("toast.renameFailedDesc"),
        color: "danger",
      });
    }
  };
  ...
  const handleSubmitAddTunnel = async () => {
    ...
    if (!portalUrl.startsWith("portal://")) {
      addToast({
        title: t("toast.portalUrlRequired"),
        description: t("toast.portalUrlRequiredDesc"),
        color: "warning",
      });
      ...
      addToast({
        ...
        description:
          err instanceof Error
            ? err.message
            : t("toast.createTunnelFailedDesc"),
        color: "danger",
      });
    }
  };
  ...
  const handleRefreshTunnels = async (endpointId: number) => {
    try {
      ...
      addToast({
        ...
        description:
          err instanceof Error ? err.message : t("toast.refreshFailedDesc"),
        color: "danger",
      });
    }
  };
    ...
    <div className=" space-y-6">
      ...
      <GuidedAddModal
        ...
        onCopyCommand={(command) =>
          copyToClipboard(
            command,
            t("toast.copyGuidedInstallSuccess"),
            showManualCopyModal,
          )
        }
        onOpenChange={onGuidedAddOpenChange}
          ...
          {(onClose) => (
            <>
              ...
              <ModalBody>
                <div className="flex flex-col gap-4">
                  <p className="text-small text-default-500">
                    {t("importModal.validateDesc", {
                      count: importValidateResults.length,
                    })}
                  </p>
                  <div className="max-h-[400px] overflow-y-auto">
                    ...
                    >
                      <table className="w-full">
                        ...
                        <tbody>
                          <SortableContext
                            items={sortedValidateResults.map(
                              (_, idx) => `item-${idx}`,
                            )}
