...
// Nowhere主控SSE监听 - 直接连接到Nowhere主控的SSE接口
export function useNowhereSSE(
  ...
  useEffect(() => {
    ...
    // 构建Nowhere SSE URL: {URL}{APIPath}/events
    const sseUrl = `${endpointDetail.url}${endpointDetail.apiPath}/events`;

    console.log("[Nowhere SSE] 直接连接到:", sseUrl);
    ...
    const connectToSSE = async () => {
      try {
        ...
        console.log("[Nowhere SSE] 连接成功，开始读取流");
            ...
            for (const line of lines) {
              ...
              if (line.startsWith("data: ")) {
                ...
                try {
                  ...
                  console.log("[Nowhere SSE] 收到JSON消息:", parsedData);
                  ...
                  // 如果不是JSON，当作纯文本日志处理
                  console.log("[Nowhere SSE] 收到文本消息:", data);
        ...
        if (error instanceof Error && error.name === "AbortError") {
          console.log("[Nowhere SSE] 连接被主动取消");
