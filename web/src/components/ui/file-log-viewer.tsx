...
export const FileLogViewer: React.FC<FileLogViewerProps> = ({
  ...
  const [internalDate] = useState<string>(""); // 内部日期状态（当外部没有提供时使用）
  ...
  const fetchFileLogs = useCallback(async () => {
    if (!instanceId || !date) return; // 添加date检查
    ...
    try {
      ...
      const path = endpointId
        ? `/api/endpoints/${encodeURIComponent(endpointId)}/file-logs`
        : `/api/tunnels/${encodeURIComponent(instanceId)}/file-logs`;

      if (endpointId) query.set("instanceId", instanceId);

      const response = await fetch(buildApiUrl(`${path}?${query.toString()}`));
      ...
      const responseLogs = Array.isArray(data.logs)
        ? data.logs
        : Array.isArray(data.data?.logs)
          ? data.data.logs
          : [];

      if (data.success && responseLogs.length > 0) {
        // 转换API返回的格式到FileLogEntry格式
        const convertedLogs: FileLogEntry[] = responseLogs.map((log: any) => ({
  ...
  const handleClearLogs = useCallback(async () => {
    ...
    try {
      const path = endpointId
        ? `/api/endpoints/${encodeURIComponent(endpointId)}/file-logs/clear?${new URLSearchParams({ instanceId }).toString()}`
        : `/api/tunnels/${encodeURIComponent(instanceId)}/file-logs/clear`;
  ...
  // 响应外部触发的刷新
  useEffect(() => {
    if (triggerRefresh && triggerRefresh > 0 && instanceId) {
      fetchFileLogs();
    }
  ...
  const appendLog = useCallback(
    (logContent: string) => {
      ...
      const newLogEntries: FileLogEntry[] = lines.map((line) => ({
  ...
  const exportLogs = useCallback(() => {
    ...
    link.href = blobUrl;
    link.download = `nowhere-${instanceId}-${date || "logs"}.log`;
    link.click();
    URL.revokeObjectURL(blobUrl);

    return true;
  }, [date, instanceId, logs]);
