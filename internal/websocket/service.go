...
func (s *Service) queryAndBroadcastEndpointInfo(endpointID int64) {
	...
	// 调用nowhere client的info接口
	endpointInfo, err := nowhere.GetInfo(endpointID)
	if err != nil {
...
func (s *Service) queryAndBroadcastInstanceInfo(instanceID string) {
	...
	if s.tunnelService == nil {
		...
		if err != nil {
			...
			// 调用nowhere client的GetInstance接口
			instanceInfo, err := nowhere.GetInstance(endpointID, instanceID)
			if err != nil {
...
func (s *Service) HandleWebSocketConnection(conn *websocket.Conn, endpointIDStr string) error {
	...
	// 检查endpoint是否在缓存中存在
	if _, _, exists := nowhere.GetCache().Get(fmt.Sprintf("%d", endpointID)); !exists {
