...
func (h *VersionHandler) selectProxy() string {
	...
	probeTarget := "https://github.com/NodePassProject/NowhereDash/releases/latest"
...
func (h *VersionHandler) detectDeploymentMethod() DeploymentInfo {
	...
	if h.isRunningInDocker() {
		manualUpdateInfo := "若自动更新失败，可在宿主机执行：\n\n" +
			"1. docker pull ghcr.io/nowhereproject/nowheredash:latest\n" +
	...
	if h.isHostEnvironment() && h.isBinaryDeployment() {
		...
		manualUpdateInfo := "手动更新方式：\n\n" +
			"1. 从 GitHub 下载最新版本：\n" +
			"   https://github.com/NodePassProject/NowhereDash/releases/latest\n\n" +
			"2. 停止当前程序\n\n" +
			"3. 替换二进制文件\n\n" +
			"4. 重新启动程序"
	...
	manualUpdateInfo := "手动更新方式：\n\n" +
		"1. Docker 部署：\n" +
		"   docker pull ghcr.io/nowhereproject/nowheredash:latest\n" +
		"   docker restart <容器名称>\n\n" +
		"2. 二进制部署：\n" +
...
func (h *VersionHandler) getBinaryDownloadURL(version, proxy string) string {
	...
	case "linux":
		...
		case "amd64", "arm64", "arm":
			filename = fmt.Sprintf("NowhereDash_Linux_%s.tar.gz", archName)
		default:
			return ""
		}
	case "windows":
		switch runtime.GOARCH {
		case "amd64", "386":
			filename = fmt.Sprintf("NowhereDash_Windows_%s.zip", archName)
		default:
			return ""
		}
	default:
	...
	rawURL := fmt.Sprintf("https://github.com/NodePassProject/NowhereDash/releases/download/%s/%s", version, filename)
...
func (h *VersionHandler) getLatestRelease(proxy string) (*GitHubRelease, error) {
	apiURL := applyGitHubProxy(proxy, "https://api.github.com/repos/NodePassProject/NowhereDash/releases/latest")
...
func (h *VersionHandler) getReleaseHistory(proxy string) ([]GitHubRelease, error) {
	apiURL := applyGitHubProxy(proxy, "https://api.github.com/repos/NodePassProject/NowhereDash/releases?per_page=10")
...
func (h *VersionHandler) extractBinary(archivePath, targetDir string) (string, error) {
	...
	expectedExeName := "nowheredash"
	if runtime.GOOS == "windows" {
		expectedExeName = "nowheredash.exe"
	}
