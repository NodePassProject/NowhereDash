...
func (h *SSEHandler) HandleTestSSEEndpoint(c *gin.Context) {
	...
	// 构造 SSE URL
	sseURL := nowhere.BuildAPIBaseURL(req.URL, req.APIPath) + "/events"
	...
	if resp.StatusCode != http.StatusOK {
		msg := fmt.Sprintf("Nowhere SSE returned status code: %d", resp.StatusCode)
...
func (h *SSEHandler) HandleTestSSEEndpointWithVersion(c *gin.Context) {
	...
	nowhere.GetCache().Set(fmt.Sprintf("%d", tempEndpointID), baseURL, req.APIKey)
	defer nowhere.GetCache().Delete(fmt.Sprintf("%d", tempEndpointID))
	...
	if err != nil {
		...
		if err != nil {
			...
			if resp.StatusCode != http.StatusOK {
				sseStatus = "failed"
				sseError = fmt.Sprintf("Nowhere SSE returned status code: %d", resp.StatusCode)
...
// GET /api/sse/nowhere-proxy?endpointId=<base64-encoded-config>
func (h *SSEHandler) HandleNowhereSSEProxy(c *gin.Context) {
	// 解析端点配置
	...
	config.APIPath = nowhere.NormalizeAPIPath(config.APIPath)
	...
	// 发送连接成功消息
	fmt.Fprintf(c.Writer, "data: %s\n\n", `{"type":"connected","message":"Nowhere SSE proxy connected successfully"}`)
	c.Writer.Flush()

	// 构造Nowhere SSE URL
	sseURL := nowhere.BuildAPIBaseURL(config.URL, config.APIPath) + "/events"
	log.Infof("[Nowhere SSE Proxy] 连接到: %s", sseURL)
	log.Infof("[Nowhere SSE Proxy] 配置信息: URL=%s, APIPath=%s", config.URL, config.APIPath)
	...
	request, err := http.NewRequestWithContext(ctx, http.MethodGet, sseURL, nil)
	if err != nil {
		log.Errorf("[Nowhere SSE Proxy] 创建请求失败: %v", err)
	...
	resp, err := client.Do(request)
	if err != nil {
		log.Errorf("[Nowhere SSE Proxy] 连接失败: %v", err)
		fmt.Fprintf(c.Writer, "data: %s\n\n", `{"type":"error","message":"Failed to connect to Nowhere"}`)
		c.Writer.Flush()
		return
	}
	defer resp.Body.Close()
	...
	if resp.StatusCode != http.StatusOK {
		log.Errorf("[Nowhere SSE Proxy] Nowhere返回状态码: %d", resp.StatusCode)
		fmt.Fprintf(c.Writer, "data: %s\n\n", fmt.Sprintf(`{"type":"error","message":"Nowhere returned status code: %d"}`, resp.StatusCode))
		c.Writer.Flush()
		return
	}
	...
	if !strings.Contains(contentType, "text/event-stream") {
		log.Errorf("[Nowhere SSE Proxy] 无效的Content-Type: %s", contentType)
		fmt.Fprintf(c.Writer, "data: %s\n\n", `{"type":"error","message":"Invalid Content-Type"}`)
		c.Writer.Flush()
		return
	}

	log.Infof("[Nowhere SSE Proxy] 连接成功，开始转发事件")
	...
	for scanner.Scan() {
		...
		select {
		case <-ctx.Done():
			log.Infof("[Nowhere SSE Proxy] 连接已关闭")
			return
		default:
		}
		...
		if _, err := fmt.Fprintf(c.Writer, "%s\n", line); err != nil {
			log.Errorf("[Nowhere SSE Proxy] 写入响应失败: %v", err)
			return
		}
		...
		// 记录所有接收到的行
		if line == "" {
			log.Debugf("[Nowhere SSE Proxy] 收到空行（事件分隔符）")
		} else if strings.HasPrefix(line, "data: ") {
			log.Infof("[Nowhere SSE Proxy] 收到并转发数据: %s", line[6:]) // 去掉"data: "前缀显示
		} else {
			log.Debugf("[Nowhere SSE Proxy] 收到其他行: %s", line)
		}
	}

	if err := scanner.Err(); err != nil {
		log.Errorf("[Nowhere SSE Proxy] 读取响应失败: %v", err)
		fmt.Fprintf(c.Writer, "data: %s\n\n", `{"type":"error","message":"Failed to read response"}`)
		c.Writer.Flush()
	}

	log.Infof("[Nowhere SSE Proxy] 连接结束")
}
