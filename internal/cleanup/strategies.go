...
func (s *OrphanDataCleanupStrategy) Execute(ctx context.Context, db *gorm.DB, config *CleanupConfig) (*CleanupResult, error) {
	...
	if orphanTunnelCount > 0 {
		...
		for orphanTunnelCount > 0 {
			...
			err = db.Model(&models.Tunnel{}).
				Where("endpoint_id NOT IN (SELECT id FROM endpoints)").
				Limit(batchSize).Pluck("id", &tunnelIDs).Error
			if err == nil {
				if len(tunnelIDs) == 0 {
					break
				}
				err = subscription.NewService(db).AccountTunnels(tunnelIDs)
			}
			if err == nil {
				err = db.Where("id IN ?", tunnelIDs).Delete(&models.Tunnel{}).Error
			}
...
func (s *DeletedEndpointsCleanupStrategy) Execute(ctx context.Context, db *gorm.DB, config *CleanupConfig) (*CleanupResult, error) {
	...
	if len(deletedEndpoints) > 0 {
		...
		for _, endpoint := range deletedEndpoints {
			var tunnelIDs []int64
			if err = db.Unscoped().Model(&models.Tunnel{}).Where("endpoint_id = ?", endpoint.ID).Pluck("id", &tunnelIDs).Error; err != nil {
				log.Errorf("查询端点 %d 的隧道失败: %v", endpoint.ID, err)
				continue
			}
			if err = subscription.NewService(db).AccountTunnels(tunnelIDs); err != nil {
				log.Errorf("结算端点 %d 的订阅流量失败: %v", endpoint.ID, err)
				continue
			}
