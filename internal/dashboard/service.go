...
func (s *Service) GetStats(timeRange TimeRange) (*DashboardStats, error) {
	...
	tunnelTypesQuery := s.db.Table("tunnels").
		Select(`
			COUNT(CASE WHEN type = 'portal' THEN 1 END) as portal,
			COUNT(*) as total
		`)
	...
	stats.TunnelTypes.Portal = tunnelTypesResult.Portal
	...
	topTunnelsQuery := s.db.Table("tunnels").
		Select("id, name, type, (tcp_rx + tcp_tx + udp_rx + udp_tx) as total_traffic").
