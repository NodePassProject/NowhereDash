...
import (
	...
	applog "NowhereDash/internal/log"
...
func StandardMigrate(db *gorm.DB) error {
	...
	if err := dropLegacySubscriptionEnabledColumn(db); err != nil {
		return err
	}
...
// before GORM migrates the current model. Older databases may still have this
// NOT NULL column without a default, which would otherwise reject new rows.
func dropLegacySubscriptionEnabledColumn(db *gorm.DB) error {
	legacySubscription := &models.PortalSubscription{}
	if !db.Migrator().HasTable(legacySubscription) || !db.Migrator().HasColumn(legacySubscription, "enabled") {
		return nil
	}

	log.Printf("删除已废弃的订阅 enabled 字段")
	if db.Dialector.Name() == dialect.NamePostgres {
		return db.Exec(`ALTER TABLE "portal_subscriptions" DROP COLUMN IF EXISTS "enabled"`).Error
	}

	return db.Exec(`ALTER TABLE "portal_subscriptions" DROP COLUMN "enabled"`).Error
}
