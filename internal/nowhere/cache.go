...
func NormalizeAPIPath(apiPath string) string {
	...
	return "/" + strings.Trim(apiPath, "/")
}

func BuildAPIBaseURL(rawURL, apiPath string) string {
	return strings.TrimRight(strings.TrimSpace(rawURL), "/") + NormalizeAPIPath(apiPath)
}
...
func InitializeCache(db *gorm.DB) error {
	...
	for _, endpoint := range endpoints {
		globalCache.items[fmt.Sprintf("%d", endpoint.ID)] = &CacheItem{
			BaseURL: BuildAPIBaseURL(endpoint.URL, endpoint.APIPath),
