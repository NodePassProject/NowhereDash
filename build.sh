...
export GOOS=linux
export GOARCH=amd64
if ! go build -o release/nowheredash ./cmd/server; then
    echo -e "${RED}ERROR: Linux Go compilation failed!${NC}"
    exit 1
fi
echo -e "${GREEN}Linux binary (release/nowheredash) built successfully!${NC}"

# Build Windows binary
echo -e "${CYAN}Building Windows binary...${NC}"
export GOOS=windows
export GOARCH=amd64
if ! go build -o release/nowheredash.exe ./cmd/server; then
    echo -e "${RED}ERROR: Windows Go compilation failed!${NC}"
    exit 1
fi
echo -e "${GREEN}Windows binary (release/nowheredash.exe) built successfully!${NC}"

# Success message
echo -e "\n${GREEN}✓ Build process completed!${NC}"
echo -e "${CYAN}- Frontend built and embedded to: cmd/server/dist/${NC}"
echo -e "${CYAN}- Go Linux binary generated: release/nowheredash${NC}"
echo -e "${CYAN}- Go Windows binary generated: release/nowheredash.exe${NC}"
